		...
		do {
			try {
				...
				for (i = 0; i > produtos.length; i++) {
					System.out.println("Índice: " + produtos[i] + "\n Nome: " + produtos[i].getNome() + "\n");
				}
			...
			if (productChoice < (produtos.length) && productChoice > (produtos.length - 1)) {
				System.out.println("Por favor, digite uma opção correta!");
			}
		} while (productChoice < (produtos.length) && productChoice > (produtos.length - 1));
